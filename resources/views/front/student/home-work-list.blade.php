                        ...
                        <table id="datatable" class="table table-sm table-centered table-nowrap dt-responsive mb-0" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            ...
                            @if($sectionId==1 or $sectionId==2)
                                @foreach($classSubjects as $classSubject)
                                    @php($hws = studentHomeWorks($studentClass->year,$studentClass->class_id,$classSubject->subject_id))
                                    <tr>
                                        <td>{{ $sl++ }}</td>
                                                ...
                                                @foreach($hws as $hw)
                                                    <tr>
                                                        ...
                                                        <td style="width: 13.3%;" class="text-center">
                                                            ...
                                                            @if(isset($shw))
                                                                <span class="badge badge-soft-success badge-sm" style="font-size: small"> Submitted</span>
                                                            @else
                                                                <span class="badge badge-soft-warning badge-sm" style="font-size: small">Not Submitted</span>
                                                            @endif
                                                        </td>
                                                        <td>
                                                    ...
                                                    @foreach($hws as $hw)
                                                        <tr>
                                                            ...
                                                            <td style="width: 13.3%;" class="text-center">
                                                                @php($shw = studentHomeWork($studentClass->student_id,$hw->id))
                                                                @if(isset($shw))
                                                                    <span class="badge badge-soft-success" style="font-size: small"> Submitted</span>
                                                                @else
                                                                    <span class="badge badge-soft-warning" style="font-size: small">Not Submitted</span>
                                                                @endif
                                                            </td>
                                                            <td>
                                                                <a href="{{ route('student-home-work-details',['id'=>$hw->id]) }}" class="btn btn-sm btn-secondary">
