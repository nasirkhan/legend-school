    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <h4 class="mb-0 font-size-18">{{ $student->name }}'s Subject Wise Syllabus</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{ route('student-profile') }}">Student Area</a></li>
                        <li class="breadcrumb-item active"><a href="{{ route('student-syllabus') }}">{{ 'Syllabus' }}</a></li>
                    </ol>
                </div>
                                ...
                                @foreach($classSubjects as $classSubject)
                                    <tr>
                                        ...
                                        @foreach($exams as $exam)
                                            ...
                                            <td>
                                                @if(isset($syllabus))
                                                    @if($syllabus->syllabus_detail != null or $syllabus->syllabus_detail == '')
                                                        <p class="m-0">{!! $syllabus->syllabus_detail !!}</p>
                                                    @endif

{{--                                                    @if($syllabus->attachment_url)--}}
{{--                                                        <a href="{{ asset($syllabus->attachment_url) }}" target="_blank">{{ 'Attachment' }}</a>--}}
{{--                                                    @endif--}}
                                                @else
                                                    Comming Soon
                                                @endif
                                    ...
                                    @if($status===true)
                                        <tr>
                                            ...
                                            @foreach($exams as $exam)
                                                ...
                                                <td>
                                                    @if(isset($syllabus))
                                                        @if($syllabus->exam_date !== null)
                                                            <b>Exam Date :  {{ dateFormat($syllabus->exam_date,'jS M Y') }}</b>
                                                        @else
                                                            <b>Exam Date :  Not Declared</b>
                                                        @endif
