<div class="card-body" id="addForm">
    <h5 class="card-title text-primary font-weight-bold mb-3">HW Add Form <a href="{{ route('teacher-class-wise-hw-list') }}" class="btn btn-primary btn-sm"> Go To Home Work List</a></h5>
    <form class="" action="{{ route('teacher-hw-save') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
...
{{--                            <option value="{{ $subject->id }}" {{ Session::get('subject_id') == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>--}}
            ...
            <div class="col-lg-12 mb-3">
                <div class="input-group">
                    ...
                    <input type="text" name="title" class="form-control" placeholder="Chapter Name or Topic Name etc." required/>
                </div>
            </div>
