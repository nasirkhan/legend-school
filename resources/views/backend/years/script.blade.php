<script>
    ...
    $("#addForm form").submit(function (e) {
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                let tr = table.row.add( [ sl, year, status, action ] ).draw().node();
    ...
    $("#editForm form").submit(function (e) {
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                temp[0] = response.sl; temp[1] = response.year;
    ...
    function editForm(sl){
      $("#addForm").hide();
      $("#editForm").show();
      $("#editForm form input[name=year]").val(data.year);
    ...
    function editFormAfterUpdate(){
      $("#addForm").hide();
      $("#editForm").show();
      $("#editForm form input[name=year]").val(updatedData.year);
    ...
    function statusUpdateConfirmation(id,sl){
      Swal.fire({
        title: 'Are you sure?',
        text: "Do you want to update this session status ?",
        icon: 'warning',
    ...
    function statusUpdate(id,sl) {
      $(".loader").show();
      $.get("{{ route('year-status-update') }}",{id:id,sl:sl}, (response)=>{
        updatedData = response;
        let table = $("#datatable").DataTable();
        let temp = table.row((sl-1)).data();
        temp[0] = sl; temp[1] = response.year;
        temp[2] = '<span class="badge badge-pill badge-soft-'+response.badge+' font-size-12">'+response.badge_txt+'</span>';
    ...
    function itemDelete(id,sl) {
      $(".loader").show();
      $.get("{{ route('year-delete') }}",{id:id,sl:sl}, (response)=>{
          if (response.success){
