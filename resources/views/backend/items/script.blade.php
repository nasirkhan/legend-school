<script>
    ...
    function edit(obj){
        ...
        $("#editForm select[name='used_for']").val(item.used_for)
        $("#editForm select[name='billing_cycle']").val(item.billing_cycle)
        $("#editForm").show()
    }
    ...
    function statusUpdate(id,sl) {
      $(".loader").show();
      $.get("{{ route('item-status-update') }}",{id:id,sl:sl}, (response)=>{
          // console.log(response);
          updatedData = response;

        let table = $("#datatable").DataTable();
        let temp = table.row((sl-1)).data();
        temp[0] = sl; temp[1] = response.name; temp[2] = response.used_for_txt; temp[3] = response.billing_cycle_txt;
        temp[4] = '<span class="badge badge-pill badge-soft-'+response.badge+' font-size-12">'+response.badge_txt+'</span>';
        temp[5] = '<button onclick="statusUpdateConfirmation('+id+','+sl+')" class="mr-1 btn btn-sm btn-'+response.btn+'"><i class="fa fa-arrow-'+response.arrow+'"></i></button>' +
    ...
    function itemDeleteConfirmation(id,sl){
        Swal.fire({
            title: 'Are you sure?',
            text: "Do you want to delete this Transaction Item?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    function itemDelete(id,sl) {
        $(".loader").show();
        $.get("{{ route('item-delete') }}",{id:id,sl:sl}, (response)=>{
            if (response.success){
                console.log(response);
                let table = $("#datatable").DataTable();
