<script>
    ...
    $("#editForm form").submit(function (e) {
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                temp[0] = response.sl; temp[1] = response.name; temp[2] = response.position;
    ...
    function editForm(sl){
      ...
      $("#editForm form input[name=position]").val(data.position);
    ...
    function statusUpdateConfirmation(id,sl){
      Swal.fire({
        title: 'Are you sure?',
        text: "Do you want to update this menu status ?",
        icon: 'warning',
    ...
    function statusUpdate(id,sl) {
      $(".loader").show();
      $.get("{{ route('menu-status-update') }}",{id:id,sl:sl}, (response)=>{
        updatedData = response;
        let table = $("#datatable").DataTable();
    ...
    function itemDelete(id,sl) {
      $(".loader").show();
      $.get("{{ route('menu-delete') }}",{id:id,sl:sl}, (response)=>{
          if (response.success){
