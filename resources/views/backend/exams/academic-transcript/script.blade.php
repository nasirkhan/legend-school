<script>
    ...
    function getClassWiseExamsForTranscriptSettings(){
        let year = $('select[name=year]').val();
        let section_id = $('select[name=section_id]').val();
        let class_id = $('select[name=class_id]').val();
        let exam_id = $('select[name=exam_id]').val();
        if (year && section_id && class_id && exam_id){
            $('.loader').show();
            $.get("{{ route('get-exam-list-for-transcript-settings') }}",{
                year:year, section_id:section_id, class_id:class_id, exam_id:exam_id, from:'add'
