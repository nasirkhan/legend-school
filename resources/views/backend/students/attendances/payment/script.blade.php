<script>
    ...
    function getBatchWiseStudent(){
        ...
        if (year && session_id && class_id && batch_id){
            ...
            $.get("{{ route('attendance-form') }}",{
                year:year, session_id:session_id, class_id:class_id, batch_id:batch_id, from:'payment', type:'add'
            },(response)=>{
                $('.loader').hide();
    ...
    function getBatchWiseAttendanceReport(){
        ...
        if (year && session_id && class_id && batch_id && from && to){
            $('.loader').show();
            $.get("{{ route('attendance-form') }}",{
                year:year, session_id:session_id, class_id:class_id, batch_id:batch_id, start:from, end:to, from:'payment', type:'view'
            },(response)=>{
                $('.loader').hide();
                $('#table').empty().append(response);
            }).then((response)=>{
