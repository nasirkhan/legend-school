<script>
    ...
    $("#editForm form").submit(function (e) {
      ...
      $.ajax({
        ...
        success: (response) => {
          ...
          temp[0] = response.sl; temp[1] = response.category; temp[2] = response.name;
      ...
      $.ajax({
        ...
        success: (response) => {
          ...
          let table = $("#datatable").DataTable();
            ...
            let category = '<td>'+response.category+'</td>';
            let subCategory = '<td>'+response.name+'</td>';
           ...
           let tr = table.row.add( [ sl, category, subCategory, status, action ] ).draw().node();
    ...
    function statusUpdateConfirmation(id,sl){
      Swal.fire({
        title: 'Are you sure?',
        text: "Do you want to update this sub category status ?",
        icon: 'warning',
    ...
    function statusUpdate(id,sl) {
      $(".loader").show();
      $.get("{{ route('sub-category-status-update') }}",{id:id,sl:sl}, (response)=>{
        updatedData = response;
        let table = $("#datatable").DataTable();
        let temp = table.row((sl-1)).data();
        temp[0] = sl; temp[1] = response.category; temp[2] = response.name;
        temp[3] = '<span class="badge badge-pill badge-soft-'+response.badge+' font-size-12">'+response.badge_txt+'</span>';
    ...
    function itemDelete(id,sl) {
      $(".loader").show();
      $.get("{{ route('sub-category-delete') }}",{id:id,sl:sl}, (response)=>{
          if (response.success){
