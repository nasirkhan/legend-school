<script>
    ...
    function edit(obj,sl) {
      data = JSON.parse(obj);
      $('#edit select[name=from]').val(data.from);
      $('#edit select[name=to]').val(data.to);
      $('#edit input[name=factor]').val(data.times);
    ...
    function statusUpdateConfirmation(id,sl){
      Swal.fire({
        title: 'Are you sure?',
        text: "Do you want to update this item status ?",
        icon: 'warning',
    ...
    function itemDelete(id,sl) {
      $(".loader").show();
      $.get("{{ route('unit-conversion-delete') }}",{id:id,sl:sl}, (response)=>{
          if (response.success){
