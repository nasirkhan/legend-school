<div class="card-body" id="addForm">
    <h5 class="card-title text-primary mb-3">Add New Client</h5>
                ...
                <div class="form-group">
                    ...
                    <select name="type" class="form-control" required>
                        ...
                        @foreach(activeClientTypes() as $type)
                            <option value="{{ $type->name }}" {{ old('type')==$type->name ? 'selected':'' }}>Client Type: {{ siteInfo('language')=='bengali'? $type->bn_name:$type->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
