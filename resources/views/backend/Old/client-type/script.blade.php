<script>
    ...
    const updatedDataCollection = new Set();
    ...
    function editForm(sl){
      ...
      $("#editForm form input[name=bn_name]").val(data.bn_name);
      $("#editForm form input[name=id]").val(data.id);
      $("#editForm form input[name=sl]").val(sl);
    }

    function editFormAfterUpdate(id){
      $("#addForm").hide();
      $("#editForm").show();

        updatedDataCollection.forEach (function(value) {
            if(value.id === id){return updatedData = value;}
        });

      $("#editForm form input[name=name]").val(updatedData.name);
      $("#editForm form input[name=bn_name]").val(updatedData.bn_name);
      ...
      $.ajax({
        ...
        success: (response) => {
          updatedData = response;
            var index = [...updatedDataCollection].findIndex(obj => obj.id === response.id);
            if (index < 0){
                updatedDataCollection.add(response);
            }else{
                updatedDataCollection.forEach((value)=>{
                    if(value.id===response.id){updatedDataCollection.delete(value);}
                });
                updatedDataCollection.add(response);
            }
          let table = $("#datatable").DataTable();
          let temp = table.row((response.sl-1)).data();
          temp[0] = response.sl; temp[1] = response.name; temp[2] = response.bn_name;
          ...
          temp[4] = '<button onclick="statusUpdateConfirmation('+response.id+','+response.sl+')" class="mr-1 btn btn-sm btn-'+response.btn+'"><i class="fa fa-arrow-'+response.arrow+'"></i></button>' +
            '<button onclick="editFormAfterUpdate('+response.id+')" class="mr-1 btn btn-sm btn-primary"><i class="fa fa-edit"></i></button>' +
    ...
    $("#addForm form").submit(function (e) {
      ...
      $.ajax({
        ...
        success: (response) => {
            ...
            let type_name = '<td>'+response.name+'</td>';
            let type_bn_name = '<td>'+response.bn_name+'</td>';
           ...
           let tr = table.row.add( [ sl, type_name, type_bn_name, status, action ] ).draw().node();
    ...
    function statusUpdate(id,sl) {
      $(".loader").show();
      $.get("{{ route('client-type-status-update') }}",{id:id,sl:sl}, (response)=>{
        updatedData = response;
        let table = $("#datatable").DataTable();
        let temp = table.row((sl-1)).data();
        temp[0] = sl; temp[1] = response.name; temp[2] = response.bn_name;
        temp[3] = '<span class="badge badge-pill badge-soft-'+response.badge+' font-size-12">'+response.badge_txt+'</span>';
    ...
    function itemDelete(id,sl) {
      $(".loader").show();
      $.get("{{ route('client-type-delete') }}",{id:id,sl:sl}, (response)=>{
          if (response.success){
