<div class="card-body">
    <h5 class="card-title text-primary mb-3">Select Year and Month and then click on <kbd>Get</kbd> button to get report</h5>
    <form class="" action="{{ route('item-wise-expense-report') }}" method="POST">
        @csrf
        <div class="row">
            ...
            <div class="col-lg-4 pr-lg-0">
                <div class="input-group">
                    <div class="input-group-prepend">
                        <span class="input-group-text">Transaction Item</span>
                    </div>
                    <select class="form-control" name="expense_item_id" required>
                        <option value="">--Select--</option>
                        <option value="all" {{ Session::get('expense_item_id') == 'all' ? 'selected':'' }}>All Items</option>
                        @foreach($expenseItems as $expenseItem)
                            <option value="{{ $expenseItem->id }}" {{ Session::get('expense_item_id') == $expenseItem->id ? 'selected':'' }}>{{ $expenseItem->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
{{--            </div>--}}
            ...
            <div class="col-lg-1">
                <button type="submit" class="btn btn-block btn-primary"><i class="fa fa-file-alt"></i> Get</button>
            </div>
        </div>
    </form>
</div>
