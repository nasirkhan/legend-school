<table id="datatable" class="table table-centered table-nowrap dt-responsive mb-0" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
    ...
    @foreach($pages as $page)
        ...
        <tr>
            ...
            <td class="text-right">
                <a href="{{ route('update-page-status',['id'=>$page->id]) }}" onclick="return confirm('If you want to update status, press OK')" class="btn btn-sm btn-{{ $page->status==1?'success':'warning' }}">
                    <i class="fa fa-arrow-{{ $page->status==1?'up':'down' }}"></i>
                </a>
                <a href="{{ route('page-edit',['id'=>$page->id]) }}"  class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></a>
                <a href="{{ route('page-delete',['id'=>$page->id]) }}" onclick="return confirm('If you want to delete this page, press OK')" class="btn btn-sm btn-danger" id="sa-params"><i class="fa fa-trash-alt"></i></a>
            </td>
        </tr>
    @endforeach
    </tbody>
