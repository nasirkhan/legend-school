<script>
    ...
    function statusUpdateConfirmation(id,sl){
      Swal.fire({
        title: 'Are you sure?',
        text: "Do you want to update this subject status ?",
        icon: 'warning',
    ...
    function statusUpdate(id,sl) {
      $(".loader").show();
      $.get("{{ route('subject-status-update') }}",{id:id,sl:sl}, (response)=>{
        updatedData = response;
        let table = $("#datatable").DataTable();
    ...
    function itemDelete(id,sl) {
      $(".loader").show();
      $.get("{{ route('subject-delete') }}",{id:id,sl:sl}, (response)=>{
          if (response.success){
              console.log(response);
              let table = $("#datatable").DataTable();
...
<script>
    ...
    function addItem(){
        index++;
        let classId = $("form select[name=class_id]").val();
        let item = '<div class="row mb-2 item" style="position:relative;" id="row'+index+'">' +
            '<button type="button" onclick="removeItem('+index+')" class="row-delete-btn">&times;</button>' +

            '<div class="col-lg-6 pr-lg-0"><input type="text" name="name['+index+']" placeholder="Subject Name" class="form-control" required/></div>'+
            '<div class="col-lg-4 pr-lg-0"><input type="text" name="code['+index+']" placeholder="Subject Code" class="form-control" required/></div>'+
            '<div class="col-lg-2"><input type="number" min="0" step="1" name="serial['+index+']" placeholder="Serial" class="form-control" required/></div>'+
