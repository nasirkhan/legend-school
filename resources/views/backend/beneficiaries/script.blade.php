<script>
    ...
    function edit(obj){
        ...
        $("#editForm input[name='contact_number']").val(item.contact_number)
        $("#editForm select[name='beneficiary_type_id']").val(item.type_id)
        $("#editForm").show()
    }
    ...
    function statusUpdateConfirmation(id,sl){
        Swal.fire({
            title: 'Are you sure?',
            text: "Do you want to update this Beneficiary status ?",
            icon: 'warning',
    ...
    function statusUpdate(id,sl) {
      $(".loader").show();
      $.get("{{ route('beneficiary-status-update') }}",{id:id,sl:sl}, (response)=>{
          // console.log(response);
          updatedData = response;

        let table = $("#datatable").DataTable();
        let temp = table.row((sl-1)).data();
        temp[0] = sl; temp[1] = response.name; temp[2] = response.contact_number;
    ...
    function itemDelete(id,sl) {
        $(".loader").show();
        $.get("{{ route('beneficiary-delete') }}",{id:id,sl:sl}, (response)=>{
            if (response.success){
                console.log(response);
                let table = $("#datatable").DataTable();
